...
RELEASE_COUNT       = 55     # encoder count at which forward drive is cut
...
TIMEOUT_S           = 20.0  # per-state safety timeout (s)
...
def state_launch_reversed():
    ...
    while True:
        ...
        if count >= -60:
            ...
            break
        if time.perf_counter() - t0 > TIMEOUT_S:
            print("  [LAUNCH - Reversed] Timeout!")
            break
        time.sleep(0.0005)   # tight poll for accurate release timing
    print(f"  [LAUNCH - Reversed] Drive cut at count={count}.")
